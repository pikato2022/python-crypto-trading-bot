...
async def main():
    ...
    asks_all_df = to_type(pd.DataFrame(
        columns=["price", "quantity", "num_of_order"]), dtypes)
    bid_all_df = to_type(pd.DataFrame(
        columns=["price", "quantity", "num_of_order"]), dtypes)
    in_position = False
    while next(consumer):
        msg = next(consumer)
        ...
        if asks_all_df.shape[0] > 10:
            in_position = await moving_average_5_strategy(asks_all_df, in_position)
